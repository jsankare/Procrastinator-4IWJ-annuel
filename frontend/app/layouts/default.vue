<template>
  <div class="min-h-screen bg-primary text-text">
    <nav
      class="sticky lg:top-2.5 z-50 bg-secondary/95 backdrop-blur supports-backdrop-filter:bg-secondary/80 mx-auto max-w-7xl sm:px-6 lg:px-8 lg:rounded-4xl border border-white/10">
      <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <div class="hidden md:flex items-center gap-6">
          ...
          <div v-if="user" class="flex items-center gap-3 pl-3 border-l border-white/10">
            <div class="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/5">
              <div
                class="w-7 h-7 rounded-full bg-accent/20 border border-accent/30 flex items-center justify-center text-accent font-semibold text-sm">
        ...
        <button class="md:hidden flex items-center justify-center w-10 h-10 rounded-md hover:bg-white/10"
          @click="menuOpen = !menuOpen">
          <svg v-if="!menuOpen" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="2"
      ...
      <transition enter-active-class="transition duration-150 ease-out" enter-from-class="opacity-0 scale-95"
        ...
        <div v-if="menuOpen"
          ...
          <NuxtLink v-if="user" to="/profile" class="hover:text-accent transition-colors py-2"
            @click="menuOpen = false">Profil</NuxtLink>
          <AdminDropdown />
          ...
          <div v-if="user" class="pt-3 border-t border-white/10 flex flex-col gap-3">
            <div class="flex items-center gap-3 px-3 py-2 rounded-lg bg-white/5">
              <div
                class="w-8 h-8 rounded-full bg-accent/20 border border-accent/30 flex items-center justify-center text-accent font-semibold">
                {{ user.username.charAt(0).toUpperCase() }}
              </div>
...
import AdminDropdown from '~/components/global/AdminDropdown.vue'
import BossModeExcel from '~/components/global/BossModeExcel.vue'
