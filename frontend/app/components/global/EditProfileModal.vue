<template>
  <div v-if="show" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
    <div class="bg-secondary rounded-xl shadow-xl p-8 w-full max-w-md relative">
      <button @click="$emit('close')" class="absolute top-4 right-4 text-xl text-accent hover:text-accent/80">&times;</button>
      ...
      <form @submit.prevent="submit">
        <div class="flex flex-col items-center mb-6">
          <label class="relative cursor-pointer group" title="Changer l'avatar">
            ...
            <span class="w-24 h-24 rounded-full overflow-hidden border-4 border-accent shadow-lg flex items-center justify-center bg-primary">
        ...
        <div class="mb-4">
          ...
          <input v-model="form.firstName" class="w-full rounded-md p-2 bg-primary text-text border border-white/10" />
        </div>
        <div class="mb-4">
          <label class="block mb-1 font-medium">Nom</label>
          <input v-model="form.lastName" class="w-full rounded-md p-2 bg-primary text-text border border-white/10" />
        </div>
        <div class="mb-4">
          <label class="block mb-1 font-medium">Avatar (URL)</label>
          <input v-model="form.avatar" class="w-full rounded-md p-2 bg-primary text-text border border-white/10" placeholder="Coller une URL ou utiliser l'upload ci-dessus" />
        </div>
